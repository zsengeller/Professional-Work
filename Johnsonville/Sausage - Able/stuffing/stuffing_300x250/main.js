...
function addMouseHandlers(){
    // add event listeners to the banner div
    banner.addEventListener("mouseover", ctaOver);
    banner.addEventListener("mouseout", ctaOut);
}

function ctaOver(){// on cta mouse over, move the arrow by 10px
    TweenLite.to(".arrow", 0.4 ,{x:10, ease:Strong.easeIn});
}

function ctaOut(){// on cta mouse out, move the arrow back by 10px
